...
class ListTree:
    ...
    def __str__(self):
        ...
        return '<Instance of {0}, address {1}:\n{2}{3}>'.format(
            self.__class__.__name__,                         # Имя клиентского класса
            id(self),                                        # Адрес экземпляра
            self.__attrnames(self, 0),                       # Список пар name=value
            self.__listclass(self.__class__, 4)
        )
    
    def __listclass(self, aClass, ident):
        dots = '.' * ident
        ...
        if aClass in self.__visited:
            ...
            genabove = (self.__listclass(c, ident+4) for c in aClass.__bases__)
            return '\n{0}<Class {1}, address {2}:\n{3}{4}>\n'.format(
                dots,
                aClass.__name__,
                self.__attrnames(aClass, ident),
                ''.join(genabove),
                dots
            )
